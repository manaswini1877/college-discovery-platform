      ...
      <div className="relative">
        <div className="relative">
          ...
          <input
            ...
            className="w-full rounded-2xl border border-[#e5e7eb] bg-white py-4.5 pr-12 pl-12 text-sm text-[#1a1a1a] placeholder-[#6b7280] outline-none transition-all focus:border-[#1a73e8] focus:ring-1 focus:ring-[#c7d2fe] disabled:opacity-50 disabled:cursor-not-allowed"
          />
          {search && (
            <button
              onClick={() => setSearch("")}
        ...
        {isOpen && selectedColleges.length < maxSelections && (
          <div className="absolute left-0 mt-2 z-30 w-full max-h-60 overflow-y-auto rounded-2xl border border-[#e5e7eb] bg-white p-2 shadow-lg animate-in fade-in slide-in-from-top-2 duration-200 divide-y divide-[#e5e7eb]">
            {isLoading ? (
              <div className="flex items-center justify-center p-4 text-sm text-[#6b7280]">
                <div className="h-4 w-4 animate-spin rounded-full border border-[#1a73e8] border-t-transparent mr-2" />
                Loading colleges...
              </div>
            ) : colleges.length === 0 ? (
              <div className="p-4 text-center text-sm text-[#6b7280]">No colleges match your query</div>
            ) : (
              colleges.map((college) => {
                ...
                return (
                  <button
                    ...
                    className="flex w-full items-center justify-between rounded-xl px-4 py-3 text-left hover:bg-[#eff6ff] transition-colors cursor-pointer group"
                  >
                    <div className="space-y-0.5 pr-4">
                      <p className="text-sm font-bold text-[#1a1a1a] group-hover:text-[#1a73e8] transition-colors">
                        {college.name}
                      </p>
                      <div className="flex items-center gap-3 text-xs text-[#6b7280]">
                        <span className="flex items-center gap-1">
                          <MapPin className="h-3 w-3 text-[#1a73e8]" />
                          {college.city}, {college.state}
                        </span>
                        <span className="flex items-center gap-1">
                          <Landmark className="h-3 w-3 text-[#6b7280]" />
                          {college.type}
                        </span>
                      </div>
                    </div>
                    <div>
                      {checked ? (
                        <div className="flex h-5 w-5 items-center justify-center rounded-full bg-[#1a73e8] text-white">
                          <Check className="h-3.5 w-3.5" />
                        </div>
                      ) : (
                        <div className="flex h-5 w-5 items-center justify-center rounded-full border border-[#d1d5db] text-[#6b7280] group-hover:border-[#1a73e8] group-hover:text-[#1a73e8]">
